...
class PolicyGenerationTests(unittest.TestCase):
    ...
    def test_existing_http_api_tag_statements_have_exact_boundaries(self):
        statements = generate_policies()[
            "journalm8-prod-deployer-compute"
        ]["Statement"]
        read = next(
            statement
            for statement in statements
            if statement["Sid"] == "ReadProductionHttpApiTags"
        )
        reconcile = next(
            statement
            for statement in statements
            if statement["Sid"] == "ReconcileProductionHttpApiTags"
        )
        expected_resource = (
            "arn:aws:apigateway:us-east-1::/tags/"
        ...
        self.assertEqual(read["Action"], ["apigateway:GET"])
        self.assertEqual(read["Resource"], expected_resource)
        self.assertEqual(
            read["Condition"],
            {
                "StringEquals": {
        ...
        self.assertFalse(
            any(
                key.startswith("aws:RequestTag/")
                for key in read["Condition"]["StringEquals"]
            )
        )
        self.assertNotIn(
            "apigateway:Request/ApiName",
            read["Condition"]["StringEquals"],
        )

        self.assertEqual(reconcile["Effect"], "Allow")
        self.assertEqual(reconcile["Action"], ["apigateway:POST"])
        self.assertEqual(reconcile["Resource"], expected_resource)
        self.assertEqual(
            reconcile["Condition"],
            {
                "StringEquals": {
        ...
        self.assertNotIn(
            "apigateway:Request/ApiName",
            reconcile["Condition"]["StringEquals"],
        )

        resources = [statement["Resource"] for statement in statements]
        self.assertNotIn("arn:aws:apigateway:us-east-1::/tags/*", resources)
        self.assertNotIn("*", (read["Resource"], reconcile["Resource"]))

    def test_existing_http_api_tag_access_rejects_noncanonical_context(self):
        statements = generate_policies()[
            "journalm8-prod-deployer-compute"
        ]["Statement"]
        read = next(
        ...
        def authorized(
            ...
            for key, expected in condition["StringEquals"].items():
                if key.startswith("aws:ResourceTag/"):
                    tag_key = key.removeprefix("aws:ResourceTag/")
                    actual = resource_tags.get(tag_key)
            ...
            allowed_tag_keys = condition.get(
                "ForAllValues:StringEquals",
                {},
            ).get("aws:TagKeys")
            if allowed_tag_keys is not None and not set(
                request_tags
            ).issubset(set(allowed_tag_keys)):
                return False

            for key, expected_null in condition.get("Null", {}).items():
                if key == "aws:TagKeys":
                    is_null = not request_tags
                else:
                    tag_key = key.removeprefix("aws:RequestTag/")
        ...
        resource = (
            "arn:aws:apigateway:us-east-1::/tags/"
            "arn:aws:apigateway:us-east-1::/apis/j56qvbzzpe"
        )
        resource_tags = {"App": "journalm8", "Stage": "prod"}
        request_tags = {
            "App": "journalm8",
            "Stage": "prod",
            "ManagedBy": "aws-cli",
        ...
        for action, statement in (
            ("apigateway:POST", read),
            ("apigateway:GET", reconcile),
        ):
            with self.subTest(wrong_action=action):
                self.assertFalse(
                    authorized(
                        statement,
                        action,
        ...
        for invalid_resource_tags in (
            {},
            {"App": "journalm8"},
            {"App": "journalm8", "Stage": "dev"},
            {"App": "journalm8", "Stage": "staging"},
            {"App": "other", "Stage": "prod"},
        ):
            with self.subTest(resource_tags=invalid_resource_tags):
                self.assertFalse(
                    authorized(
                        read,
                        "apigateway:GET",
                        resource,
                        invalid_resource_tags,
                    )
                )
                self.assertFalse(
                    authorized(
        ...
        for missing_key in request_tags:
            with self.subTest(missing=missing_key):
                incomplete = dict(request_tags)
        ...
        for key, value in (
            ...
            with self.subTest(request_tag=key, value=value):
                invalid = dict(request_tags)
                invalid[key] = value
                self.assertFalse(
                    authorized(
                        reconcile,
        ...
        for unrelated_resource in (
            ...
            (
                "arn:aws:apigateway:us-east-1::/tags/"
                "arn:aws:apigateway:us-east-1::/v2/apis/j56qvbzzpe"
            ),
        ):
            with self.subTest(resource=unrelated_resource):
                self.assertFalse(
